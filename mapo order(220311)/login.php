...
if (isset($_POST['a_id']) && isset($_POST['a_pw'])) {
    ...
    $sql = "select a_no, a_name, a_pw, a_tel, a_sort, a_agency from admin where a_id = '$id' and a_sort = 'order' and a_status = '활성' limit 1";   //DB에서 아이디값과 비밀번호가 동일한
...
/*        $session_chk = "INSERT INTO session(m_no, s_id, s_login_time, s_session, s_ip) VALUES ('{$_SESSION['mno']}', '{$id}' ,'{$login}', '{$session}', '{$_SESSION['ip']}')";
        ...
        echo "<meta http-equiv='refresh' content = '0; url=order-list'>";
    } else {
